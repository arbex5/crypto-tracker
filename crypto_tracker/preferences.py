...
class PreferencesWindow(Adw.PreferencesWindow):
    ...
    def _build_ui(self):
        ...
        # API Key row
        self.api_key_row = Adw.EntryRow.new()
        self.api_key_row.set_title("API Key")
        self.api_key_row.set_text(self.settings.cmc_api_key)
        self.api_key_row.set_show_apply_button(True)
        ...
        # Toggle para usar CMC
        self.use_cmc_row = Adw.SwitchRow.new()
        self.use_cmc_row.set_title("Usar CoinMarketCap")
        self.use_cmc_row.set_subtitle(
            "Prioriza CMC quando disponível"
        )
        self.use_cmc_row.set_active(self.settings.use_cmc)
        self.use_cmc_row.connect("notify::active", self._on_use_cmc_changed)
        ...
        self.show_brl_row = Adw.SwitchRow.new()
        self.show_brl_row.set_title("Mostrar preço em BRL")
        self.show_brl_row.set_subtitle(
            "Exibe o valor convertido para Reais abaixo do preço em USD"
        )
        self.show_brl_row.set_active(self.settings.show_brl_price)
        self.show_brl_row.connect("notify::active", self._on_show_brl_changed)
        ...
        self.pin_row = Adw.SwitchRow.new()
        self.pin_row.set_title("Manter no topo")
        pin_supported = hasattr(Gtk.Window, 'set_keep_above')
        if pin_supported:
            self.pin_row.set_subtitle(
                "Fixa a janela do modo display sobre as outras janelas"
            )
        else:
            self.pin_row.set_subtitle(
                "Indisponível neste ambiente gráfico (Wayland)"
            )
            self.pin_row.set_sensitive(False)
        self.pin_row.set_active(self.settings.display_pinned and pin_supported)
        self.pin_row.connect("notify::active", self._on_pin_changed)
        ...
        links_group.set_title("Recursos")
        links_group.set_description(
            "Você pode baixar ícones em sites como: cryptoicons.co, coinmarketcap.com"
        )
    ...
    def _on_test_api(self, button):
        ...
        key = self.api_key_row.get_text().strip()
        if not key:
    ...
    def _on_api_key_apply(self, entry):
        ...
        if key:
            ...
            self.use_cmc_row.set_active(False)
    
    def _on_use_cmc_changed(self, row, param):
    ...
    def _on_show_brl_changed(self, row, param):
